...
function approve_pending_borrow(mysqli $conn, int $borrowId): array
{
    ...
    $approveStmt = $conn->prepare("
        UPDATE borrows
        SET status = 'borrowed', borrow_date = ?, due_date = ?, return_date = NULL
        WHERE id = ? AND status = 'pending'
    ");
    $approveStmt->bind_param('ssi', $borrowDate, $dueDate, $borrowId);
    $approveStmt->execute();
...
$pendingReturnRows = $conn->query("
    SELECT
      ...
      br.return_batch,
      br.request_batch,
      br.return_date AS request_date,
      u.fullname,
      u.username,
      u.role,
      b.title,
    ...
    WHERE br.status = 'return_requested'
      ...
      AND br.return_batch <> ''
    ORDER BY br.return_date DESC, br.id ASC
");
  ...
  <div class="stack">
    ...
    <div class="grid cards">
        ...
        <div class="stack">
          ...
          <?php while ($request = $recentRequests->fetch_assoc()): ?>
            <div class="empty-state">
              ...
              <span class="muted"><?php echo h($request['username']); ?> | Due <?php echo h(format_display_date((string) $request['due_date'])); ?></span>
            </div>
          <?php endwhile; ?>
        </div>
      </div>
      <div class="panel">
        ...
        <div class="stack">
          <div class="empty-state">Pending borrow requests must be approved before release and stock deduction.</div>
    ...
    <div class="panel">
      <div class="card-head">
        ...
        <div class="grow">
          <span class="chip">Grouped Queue</span>
          <h3 class="heading-top-md heading-card">Pending Request Batches</h3>
          <p class="muted">Each student/faculty submission is grouped here before you drill into individual titles.</p>
        </div>
      </div>
      <div class="stack flow-top-md">
        ...
        <?php foreach ($pendingBatches as $batch): ?>
          <div class="panel librarian-batch-card">
            <div class="librarian-batch-head">
              ...
              <div class="librarian-batch-meta">
                ...
                <?php if ((int) ($batch['waiting_stock_items'] ?? 0) > 0): ?>
                  <span class="chip"><?php echo (int) $batch['waiting_stock_items']; ?> waiting for stock</span>
                <?php endif; ?>
              ...
              <?php foreach ($batch['items'] as $item): ?>
                <div class="empty-state librarian-batch-item">
                  <div>
                    ...
                    <span class="muted"><?php echo h($item['author']); ?> | Request #<?php echo (int) $item['borrow_id']; ?><?php echo !empty($item['waiting_for_stock']) ? ' | Waiting for stock' : ''; ?></span>
                  </div>
                  ...
                  <?php else: ?>
                    <form method="post" class="inline-form" data-confirm="Approve this borrow request and release the book now?">
                      ...
                      <button type="submit" name="approve_borrow" value="1">Approve</button>
                    </form>
                  <?php endif; ?>
                </div>
              <?php endforeach; ?>
            ...
            <?php if ((int) ($batch['actionable_items'] ?? 0) > 1): ?>
              <form method="post" class="inline-form flow-top-md" data-confirm="Approve all available requests in this batch now?">
                ...
                <button type="submit" name="approve_batch" value="1">Approve <?php echo (int) $batch['actionable_items']; ?> Available</button>
              </form>
              <p class="muted meta-top-sm">Bulk approve will release only the requests still pending in this batch.</p>
            <?php elseif ((int) ($batch['actionable_items'] ?? 0) === 0 && (int) ($batch['waiting_stock_items'] ?? 0) > 0): ?>
              <p class="muted meta-top-sm">This batch is still pending, but every remaining item is waiting for an available copy.</p>
            <?php endif; ?>
          </div>
        <?php endforeach; ?>
      </div>
    ...
    <div class="panel">
      <div class="card-head">
        <div class="dashboard-icon icon-recent" aria-hidden="true"></div>
        <div class="grow">
          <span class="chip">Return Queue</span>
          <h3 class="heading-top-md heading-card">Pending Return Batches</h3>
          <p class="muted">Partial returns are expected. Confirm only the items physically received.</p>
        </div>
      </div>
      <div class="stack flow-top-md">
          ...
          <div class="panel librarian-batch-card">
            <div class="librarian-batch-head">
              <div>
                <strong class="label-block"><?php echo h($batch['fullname']); ?></strong>
                <span class="muted">
                  <?php echo h($batch['username']); ?> | <?php echo h(role_label($batch['role'])); ?> | <?php echo h(format_batch_reference($batch['return_batch'], 'Return Ref')); ?> |
                  <?php echo (int) $batch['requested_items']; ?> of <?php echo (int) $batch['total_items']; ?> requested for return
                </span>
              ...
              <div class="librarian-batch-meta">
                <span class="chip"><?php echo (int) $batch['outstanding_items']; ?> still outstanding</span>
            ...
            <div class="stack librarian-batch-list">
              <?php foreach ($batch['items'] as $item): ?>
                <div class="empty-state librarian-batch-item">
                  <div>
                    <strong class="label-block meta-top-sm"><?php echo h($item['title']); ?></strong>
                    <span class="muted"><?php echo h($item['author']); ?> | Return request #<?php echo (int) $item['borrow_id']; ?></span>
                  </div>
            ...
            <?php if (count($batch['items']) > 1): ?>
              <form method="post" class="inline-form flow-top-md" data-confirm="Confirm all physically received books in this return batch?">
                ...
                <button type="submit" name="confirm_return_batch" value="1">Confirm <?php echo count($batch['items']); ?> Requested</button>
              </form>
        ...
        <table>
          ...
          <tbody>
            <?php if ($borrows->num_rows === 0): ?>
              <tr><td colspan="9" class="muted">No borrow records matched your filters.</td></tr>
            <?php endif; ?>
            <?php while ($borrow = $borrows->fetch_assoc()): ?>
              ...
              <tr>
                ...
                <td><?php echo $borrow['status'] === 'pending' ? '-' : h(format_display_date((string) $borrow['due_date'])); ?></td>
                <td>
                  <span class="badge">
                ...
                <td>
                  <?php if ($borrow['status'] === 'pending'): ?>
                    <?php if ($waitingForStock): ?>
                      <span class="muted">Waiting for available copy</span>
                    <?php else: ?>
                      <form method="post" class="inline-form" data-confirm="Approve this borrow request and release the book now?">
                        <input type="hidden" name="borrow_id" value="<?php echo (int) $borrow['id']; ?>">
                        <button type="submit" name="approve_borrow" value="1">Approve Borrow</button>
                      </form>
                    <?php endif; ?>
                  ...
                  <?php else: ?>
                    <span class="muted">Active with borrower</span>
                  <?php endif; ?>
                </td>
              </tr>
            <?php endwhile; ?>
