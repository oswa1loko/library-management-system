...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'mark_read' && $notificationId > 0) {
        $markStmt = $conn->prepare("
            ...
            SET is_read = 1
            WHERE id = ? AND role = 'student' AND user_id = ? AND is_read = 0
            LIMIT 1
        ");
        ...
        $countStmt = $conn->prepare("
            ...
            FROM notifications
            WHERE role = 'student' AND user_id = ? AND is_read = 0
        ");
    ...
    if ($action === 'mark_alert_read' && $borrowId > 0) {
        ...
        $_SESSION['student_read_due_alerts'][$userId] = array_values(array_unique(array_filter(array_map('intval', $dismissedDueAlerts))));
...
foreach ($overdueBooks as $dueBook) {
    ...
    $items[] = [
        ...
        'is_read' => $isRead,
        'created_at' => 'Overdue',
        'relative_time' => 'Needs attention',
        'category_label' => 'Overdue',
        'created_at_raw' => '',
        'kind' => 'overdue',
        'destination_url' => '/librarymanage/student/borrow_return.php?borrow_id=' . $borrowId . '&from_notification=1',
        'destination_label' => 'Open borrow status',
